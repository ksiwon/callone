"""S4 노트북 TTS 학습 (§14) — per-speaker 소형 단일화자 모델.

노트북(Arc/CPU)은 CosyVoice3(CUDA) 불가 → 경량 TTS.
Piper/VITS(초경량) · MeloTTS(한국어 CPU 실시간) · GPT-SoVITS · Kokoro+KokoClone.
동일 정제 코퍼스(metadata.csv) 재사용. → models/tts_laptop/{spk}.
경량 TTS 클론이 가장 까다로움 → 데이터 풍부 살린 전용 모델이 정답(§17.2).

백엔드 repo 무거움 → 미설치 시 학습 레시피 안내(폴백).

사용:
  callone-tts-laptop --speakers A
"""
...
log = get_logger("tts_laptop")


def run(cfg: dict, speakers: list[str]) -> None:
    backend = cfg.get("backend", "piper")
    out_base = Path(cfg.get("output_dir", "models/tts_laptop"))
    for spk in speakers:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="S4 노트북 TTS 학습")
    ap.add_argument("--config", default="tts_laptop")
