# 클라우드 GPU — 실시간 음성 통화 실행 순서 (RunPod · Elice 공용, 2026-06 실측)

> 켜자마자 **위에서 아래로 그대로** 복붙. 각 단계 끝 **✅ 체크**가 통과해야 다음으로.
> 싼 단계(설치·헬스)부터 → 비싼 GPU 시간 낭비 방지. 음성 통화만(토킹헤드·학습 제외).
> **RunPod(RTX 3090/4090)** 과 **Elice(A100/H100)** 둘 다 같은 순서로 동작한다.
> 환경 차이(영속 폴더·root 여부)는 **`$CALLONE_HOME` 한 변수**로 흡수한다(1번). 근거/대안은 [DEPLOY.md](../DEPLOY.md).
...
| 환경 | GPU | VRAM | 비고 |
|---|---|---|---|
| **RunPod** | RTX **3090 / 3090 Ti / 4090** | 24GB | Community Cloud 절반가(4090 ~$0.34/hr). 토큰생성속도 거의 동급(prefill만 4090이 빠름). |
| **Elice** | **A100 / H100** | 40~80GB | VRAM 넉넉 → LLM+TTS 여유 + 나중 토킹헤드까지. 비싸지만 빠름. |

- 둘 다 **NVIDIA + CUDA 12.x** 계열 → 우리 torch(cu124)로 동작. 16GB 비추(빠듯).
- 템플릿: **순정 PyTorch / CUDA(12.x)**. ComfyUI 류 금지. CUDA 13 베이스라도 cu124 torch OK.
- 디스크: 영속 볼륨 ~40GB(실사용 ~25GB: 모델 ~10GB + llama.cpp 빌드 + 캐시).
- 접속: **RunPod** = Connect → Web terminal / JupyterLab(8888). **Elice** = 터미널/노트북.
- 빌드툴: `nvcc --version` 이 나와야 llama.cpp 빌드 가능(아래 3번). 없으면 CUDA **devel/toolkit** 포함 이미지로.

---

## 1. 코드 + 영속 폴더($CALLONE_HOME) + 환경변수  ★환경차이 흡수
```bash
# 영속 폴더 자동 선택: RunPod=/workspace(쓰기가능) / Elice 등(non-root)=홈($HOME)
if [ -d /workspace ] && [ -w /workspace ]; then export CALLONE_HOME=/workspace; else export CALLONE_HOME=$HOME; fi
echo "CALLONE_HOME=$CALLONE_HOME"          # RunPod→/workspace, Elice→/home/elicer 등

cd "$CALLONE_HOME"
git clone https://github.com/ksiwon/callone.git && cd callone

export HF_HOME=$CALLONE_HOME/hf_cache       # 모델 캐시 → Stop/Start 유지
export CALLONE_TIER=server_gpu              # GPU 강제
# 새 셸에서도 유지(값을 고정해서 기록 — 셸마다 재계산 불필요)
{ echo "export CALLONE_HOME=$CALLONE_HOME"
  echo "export HF_HOME=$CALLONE_HOME/hf_cache"
  echo "export CALLONE_TIER=server_gpu"; } >> ~/.bashrc
```
✅ **체크:** `echo $CALLONE_HOME $HF_HOME` 가 **존재하는 쓰기가능 경로**를 가리킨다.
> HF_TOKEN 불필요(9B GGUF·Qwen3-TTS·whisper 전부 비게이트).
> ⚠️ `/workspace/...` 를 잘못 박았다면: `sed -i '\#/workspace#d' ~/.bashrc` 후 위 블록 다시.
...
> Elice(non-root)서 ffmpeg apt 설치 실패해도 OK(wav 업로드는 동작). 필요시 `conda install -y ffmpeg`.

---

## 3. llama.cpp 빌드 (LLM 서버 바이너리, CUDA)
```bash
nvcc --version                                  # CUDA 컴파일러 확인(없으면 devel 이미지 필요)
bash scripts/build_llama_cuda.sh                # → $CALLONE_HOME/llama.cpp (자동: /workspace 없으면 $HOME)
```
✅ **체크:** `✅ 빌드 완료: <경로>/llama.cpp/build/bin/llama-server`
> 빌드 위치는 `$CALLONE_HOME`를 따라 자동(RunPod=/workspace, Elice=$HOME). 강제하려면 `DEST=...` 또는 `CALLONE_HOME=...`.
> Elice(non-root)서 `cmake` 없다고 멈추면: `conda install -y cmake` 후 재실행(보통 devel 이미지엔 있음).

---

## 4. 모델 다운로드 (`$CALLONE_HOME` → Stop/Start 유지)
```bash
pip install hf_transfer                          # 베이스가 HF_HUB_ENABLE_HF_TRANSFER=1 이면 필수(에러나면 설치/또는 =0)
...
huggingface-cli download HauhauCS/Qwen3.5-9B-Uncensored-HauhauCS-Aggressive \
  --include "*Q4_K_M*.gguf" --local-dir $CALLONE_HOME/models/llm_A
...
echo "export CALLONE_TTS_MODEL=$CALLONE_HOME/models/qwen3_tts" >> ~/.bashrc
```
✅ **체크:** `ls -lh $CALLONE_HOME/models/llm_A/*Q4_K_M*.gguf` (5.3GB 1개) + `ls $CALLONE_HOME/models/qwen3_tts` (config/model.safetensors 등)
...
> 참조 클립은 git 에 없으니(개인 음성) **JupyterLab/노트북 드래그 업로드** 또는 scp 로 `~/callone/data/speakers/sis/` 에 올린다.

```bash
apt-get install -y sox >/dev/null 2>&1 || sudo apt-get install -y sox >/dev/null 2>&1 || true  # 경고 제거(없어도 동작)
...
> 속도 옵션(선택): 잘 뜨면 다음 기동부터 `--flash-attn on`(bare `--flash-attn`는 최신 빌드서 에러). A100/H100 은 VRAM 여유라 `-c 16384` 로 더 길게 가능.
...
sf.write("call_tts_test.wav", y, tts.sr); print("OK", y.shape, tts.sr)
PY
```
✅ **체크:** `chunk_size=25 temperature=0.5` + `[warm] 첫청크 ~0.5~0.7s` + `call_tts_test.wav`(화자 음색).
...
# ② 전체 한 턴 (ASR→LLM→TTS→음성). Orchestrator 생성 시 워밍업 자동.
python - <<'PY'
import numpy as np, soundfile as sf
from callone.serve.orchestrator import Orchestrator
orch = Orchestrator("sis")                                     # ← 생성 시 ASR/LLM/TTS 워밍업
...
print("단계별   :", turn.timings)                              # asr/llm_first/llm_total/tts_first/first_audio (ms)
sf.write("call_reply.wav", np.concatenate(turn.audio_chunks), orch.tts.sr)
PY
```
✅ **체크:** ①감정 태그 문맥별 변화 + ②`call_reply.wav`(화자 음색·문맥 톤).
> 지연 주의: 위 측정은 **11초 클립**을 ASR 하느라 부풀려짐. 실제 통화는 짧게 말하니 더 짧다 → 정확한 병목은 8-2 벤치로.
...
(`--n-gpu-layers`, 양자화, speculative)로 깎는다. 둘 다 **목소리는 그대로** 두고 가능한 레버.
> A100/H100 은 3090/4090 보다 절대값이 빠르게 나오지만, **어느 단계가 병목인지**는 동일하게 보인다.

---

## 9. 브라우저 마이크 실시간 통화 (studio)
포트 설정/재시작 없이 **gradio 공개 링크**로 바로(RunPod·Elice 공통, 인터넷 egress 되면 됨):
```bash
...
> 포트로 직접 쓰려면(대안): 노출 포트 P 에 `PORT=P python -m studio` → RunPod Connect / Elice 노출주소.
> 풀 실시간(WebSocket·barge-in)은 `callone-serve`(:8000) + `ui`(:5173).

> ⚠️ **체감 지연:** studio 턴제는 **응답 합성이 다 끝나야** 음성이 나온다(첫음성 ms 와 별개).
> + **gradio.live 터널**이 왕복 지연을 더한다(~2초). 빠르게 하려면 ①**직접 포트**(터널 회피)
...
## Stop / 재시작
- 모델·venv·코드 전부 `$CALLONE_HOME`(RunPod=/workspace, Elice=$HOME) → **Stop 후에도 유지**. 재접속 시 `source .venv-serve/bin/activate` 만.
- `~/.bashrc` 에 박은 env(CALLONE_HOME/HF_HOME/CALLONE_TIER/CALLONE_TTS_MODEL)는 새 셸에서 자동.
- llama-server 는 Stop 시 죽음 → 재시작 후 **6번 재기동**.
- 코드 변경은 `git pull`(로컬 수정 있으면 `git checkout <file>` 후). 며칠 안 쓰면 인스턴스 Terminate(디스크 0, 다음에 이 문서 1번부터 ~30분 복구).
