...
1. EXAONE-3.5-2.4B **base GGUF**(Q4) 준비(~1.5GB): community GGUF 를 받거나, base 에서 직접 생성
   `python scripts\make_gguf.py --speaker A --quant f16 --keep-merged` 의 병합 전 base 를 재사용.
...
python C:\tools\llama.cpp\convert_lora_to_gguf.py `
    ...
    --outfile models_gguf\spk-A-lora-f16.gguf
```
> 어댑터 폴더는 `adapter_config.json` 있는 곳(루트 또는 checkpoint-*/). B 화자는 `llm_laptop\B`.

### 정석 길 — 병합본 단일 GGUF (품질 최고, base 9GB 필요)
```powershell
python scripts\make_gguf.py --speaker A --llama-cpp C:\tools\llama.cpp --quant Q4_K_M
# → models_gguf\exaone-2.4b-A-Q4_K_M.gguf (LoRA 이미 병합됨, --lora 불필요)
```
...
C:\tools\llama.cpp\llama-server.exe `
    -m models_gguf\exaone-2.4b-Q4_K_M.gguf `
    --lora models_gguf\spk-A-lora-f16.gguf `
...
**CPU 폴백(드라이버 문제 시, 안정적이나 약간 느림):**
```powershell
C:\tools\llama.cpp\llama-server.exe -m models_gguf\exaone-2.4b-Q4_K_M.gguf `
    --lora models_gguf\spk-A-lora-f16.gguf -c 4096 --host 127.0.0.1 --port 8080 --jinja
```
